...
import {
  ArrowLeft, Package, Maximize2, Building2, MapPin,
  Calendar, User, DollarSign, Edit2,
  CheckCircle2, AlertTriangle, Wrench, Clock,
} from "lucide-react";
import type { Unit, Branch } from "@/types";
...
export function UnitDetailClient({ unit: initialUnit, branch }: Props) {
...
export function UnitNotFound({ unitId }: { unitId: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-24 gap-4 text-muted-foreground">
      <AlertTriangle className="w-12 h-12 text-yellow-500" />
      <h2 className="text-xl font-semibold text-foreground">ไม่พบยูนิต</h2>
      <p className="text-sm">
        Unit ID: <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">{unitId}</code> ไม่มีในระบบ
      </p>
